...
def test_kolmogorov_smirnov(data, ks_alpha):
    ...
    for col in columns:
        ...
        # Drop NaNs and check if there are enough unique values in both samples
        sample1_clean = sample1[col].dropna()
        sample2_clean = sample2[col].dropna()

        if len(sample1_clean.unique()) <= 1 or len(sample2_clean.unique()) <= 1:
            # Skip columns with only one unique value in either sample
            print(f"Skipping KS test for {col} due to insufficient unique values")
            continue

        # Perform the Kolmogorov-Smirnov test
        ts, p_value = scipy.stats.ks_2samp(sample1_clean, sample2_clean)

        # Handle NaN p-values gracefully
        if math.isnan(p_value):
            print(f"Skipping KS test for {col} due to NaN p-value")
            continue

        # Check if the p-value passes the Bonferroni-corrected threshold
        assert p_value > alpha_prime, (
            f"KS test failed for {col}: p-value = {p_value}, expected > {alpha_prime}, "
            f"min={sample1[col].min()}, max={sample1[col].max()}, "
            f"min={sample2[col].min()}, max={sample2[col].max()}"
        )